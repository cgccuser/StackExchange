...
private val partialCommands: Map[Char, Command] = Map[Char, PartialCommand](
  'D' -> { case a -: b -: c -: s => c -: b -: a -: s },                  //reverse top three elements
  ...
  (name, command) => name -> completePartialCmd(command, name.toString)
}

val allCommands: Map[Char, Command] = fullCommands ++ partialCommands

def completePartialCmd(partialCmd: PartialCommand, name: String) =
  partialCmd.orElse { stack =>
    throw IllegalArgumentException(s"Stack is of incorrect shape for command '$name': $stack")
  }
